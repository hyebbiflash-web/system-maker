...
export default function Home() {
  ...
  const deletePlannerTodo = () => {
    ...
    addTrashItem({
      ...
      detail: [
        { label: "영역", value: `${targetTodo.area} / ${targetTodo.subArea}` },
        ...(targetTodo.dueDate ? [{ label: "마감일", value: targetTodo.dueDate }] : []),
        { label: "중요도", value: "★".repeat(targetTodo.importance) || "없음" },
        { label: "긴급도", value: "★".repeat(targetTodo.urgency) || "없음" },
        { label: "진행률", value: `${targetTodo.progress}%` },
        { label: "완료 여부", value: targetTodo.done ? "완료" : "미완료" },
      ],
    });
    setEditingTodoId(null);
    setIsTodoModalOpen(false);
  ...
  const deletePlannerSchedule = async () => {
    ...
    addTrashItem({
      ...
      detail: [
        ...(targetEvent.start?.dateTime ? [{ label: "시작", value: new Date(targetEvent.start.dateTime).toLocaleString("ko-KR") }] : []),
        ...(targetEvent.end?.dateTime ? [{ label: "종료", value: new Date(targetEvent.end.dateTime).toLocaleString("ko-KR") }] : []),
        ...(targetEvent.location ? [{ label: "장소", value: targetEvent.location }] : []),
        ...(targetEvent.description ? [{ label: "설명", value: targetEvent.description }] : []),
        ...(targetEvent.calendarSummary ? [{ label: "캘린더", value: targetEvent.calendarSummary }] : []),
      ],
    });
    setScheduleTitle("");
    setScheduleStart("");
  ...
  const createPlannerTodoFromProject = async ({
    title,
    area,
    subArea,
    dueDate,
  ...
  useEffect(() => {
    ...
    const syncGoogleTasks = async () => {
      try {
        // 1. Google Tasks에서 기존 task 가져오기
        const response = await fetch(
          "https://tasks.googleapis.com/tasks/v1/lists/@default/tasks?showCompleted=true&showHidden=false",
          { headers: { Authorization: `Bearer ${token}` } }
        );
        const data = await response.json();
        if (!response.ok) return;

        const googleTaskTodos: Todo[] = (data.items || []).map(
          (task: { id: string; title?: string; notes?: string; due?: string; status?: string }, index: number) => {
            const dueDate = task.due ? task.due.slice(0, 10) : "";
            const [taskArea, taskSubArea] = (task.notes || "").split("/").map((s) => s.trim());
        ...
        // 2. 로컬 할 일(googleTaskId 없음)을 Google Tasks로 업로드
        setTodos((currentTodos) => {
          const localOnly = currentTodos.filter((t) => !t.googleTaskId);

          const uploadLocal = async () => {
            const synced = await Promise.all(
              localOnly.map(async (todo): Promise<Todo> => {
                try {
                  ...
                  if (res.ok && result?.id) return { ...todo, googleTaskId: result.id };
                } catch { /* 실패 시 원본 유지 */ }
                return todo;
              })
            );

            // 업로드 완료 후 googleTaskId 반영
            setTodos((prev) => {
              const syncedMap = new Map(synced.map((t) => [t.id, t]));
              return prev.map((t) => syncedMap.get(t.id) ?? t);
            });
          };

          void uploadLocal();

          // 업로드 대기 중: Google Tasks 항목 병합 (로컬 유지 + Google 추가)
          const googleIds = new Set(googleTaskTodos.map((t) => t.googleTaskId));
          return [
            ...currentTodos.filter((t) => !googleIds.has(t.googleTaskId)),
            ...googleTaskTodos,
          ];
        });
      } catch (error) {
    ...
    // Google Tasks 연동: 로그인/토큰 복원 후 1회 실행
...
function ProjectPage({
  ...
  const deleteProject = (projectId: number) => {
    ...
    onTrashItem({
      ...
      detail: [
        { label: "영역", value: `${targetProject.area} / ${targetProject.subArea}` },
        { label: "상태", value: targetProject.status },
        ...(targetProject.dueDate ? [{ label: "마감일", value: targetProject.dueDate }] : []),
        { label: "진행률", value: `${targetProject.progress}%` },
        ...(targetProject.desc ? [{ label: "설명", value: targetProject.desc }] : []),
        { label: "할 일 수", value: `${targetProject.todos.length}개` },
        ...(targetProject.todos.length > 0 ? [{ label: "할 일 목록", value: targetProject.todos.map((t) => t.title).join(", ") }] : []),
      ],
    });
  };
          ...
          <div>
            ...
            <div style={{ position: "relative" as const }}>
              <input
                ...
                onFocus={(e) => {
                  (e.currentTarget as HTMLInputElement).style.borderBottom = "2px solid #1A73E8";
                  (e.currentTarget as HTMLInputElement).style.paddingBottom = "4px";
                }}
                onBlur={(e) => {
                  (e.currentTarget as HTMLInputElement).style.borderBottom = "none";
                  (e.currentTarget as HTMLInputElement).style.paddingBottom = "0";
                }}
              />
            </div>
...
function RecordPage({
  ...
  const deleteRecordMemo = (recordId: number, memoId: number) => {
    ...
    onTrashItem({
      ...
      detail: [
        { label: "분류", value: targetMemo.subCategory },
        { label: "작성일", value: targetMemo.createdAt },
        ...(targetMemo.source ? [{ label: "출처", value: targetMemo.source }] : []),
        ...(targetMemo.content ? [{ label: "내용", value: targetMemo.content }] : []),
      ],
    });
  };
...
function TrashPage({
  ...
  return (
    <div>
      ...
      {selectedTrashItem && (
        <div style={modalBackdropStyle}>
          <div style={{ ...modalBoxStyle, width: "500px", padding: "28px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", gap: "16px", marginBottom: "20px" }}>
              <div>
                <h2 style={{ fontSize: "22px", fontWeight: "800", margin: 0 }}>{selectedTrashItem.title}</h2>
                <p style={{ color: "#8A8178", margin: "6px 0 0", fontSize: "13px" }}>
                  {selectedTrashItem.section} · 삭제일 {selectedTrashItem.deletedAt}
                </p>
              </div>
            ...
            {selectedTrashItem.detail && selectedTrashItem.detail.length > 0 && (
              <div style={{
                background: "#F7F8FA",
                borderRadius: "12px",
                padding: "16px",
                ...
                <div style={{ fontSize: "11px", fontWeight: "700", color: "#8A8178", letterSpacing: "0.05em", textTransform: "uppercase" as const, marginBottom: "4px" }}>
                  삭제 전 최종 상태
                </div>
                {selectedTrashItem.detail.map((row, i) => (
                  <div key={i} style={{ display: "flex", gap: "12px", fontSize: "14px" }}>
                    <span style={{ color: "#8A8178", minWidth: "60px", flexShrink: 0 }}>{row.label}</span>
                    <span style={{ color: "#191F28", wordBreak: "break-word" as const, flex: 1 }}>{row.value}</span>
                  </div>
                ))}
              </div>
            )}
    ...
    <div>
      ...
      <div style={calendarShellStyle}>
        ...
        <div style={calendarFrameStyle}>
          ...
          {/* 할 일(Task) 행 – 구글 캘린더 스타일 */}
          {todos.some((todo) => todo.dueDate && weekDays.some((d) => new Date(`${todo.dueDate}T00:00`).toDateString() === d.toDateString())) && (
            <div style={{ ...googleCalendarGridStyle, borderBottom: "2px solid #E8EAED", background: "#FAFAFA" }}>
              <div style={{ borderRight: "1px solid #E8EAED", minWidth: "64px", display: "flex", alignItems: "flex-start", justifyContent: "center", paddingTop: "8px" }}>
                <span style={{ fontSize: "10px", color: "#8A8178", fontWeight: "600", letterSpacing: "0.03em" }}>할 일</span>
              </div>
              {weekDays.map((date) => {
                const dateTodos = todos.filter((todo) => todo.dueDate && new Date(`${todo.dueDate}T00:00`).toDateString() === date.toDateString());
                return (
                  <div key={date.toDateString()} style={{ borderRight: "1px solid #E8EAED", padding: "4px 5px", display: "flex", flexDirection: "column" as const, gap: "3px", minHeight: dateTodos.length > 0 ? undefined : "32px" }}>
                    {dateTodos.map((todo) => (
                      <div
                        key={todo.id}
                        style={{
                          ...
                          padding: "3px 6px",
                          borderRadius: "6px",
                          background: todo.done ? "transparent" : "#EEF3FF",
                          border: todo.done ? "1px solid #E0E0E0" : "none",
                          cursor: "default",
                        }}
                      >
                        {/* 동그라미 체크 버튼 */}
                        <button
                          ...
                          style={{
                            ...
                            height: "16px",
                            borderRadius: "50%",
                            border: `2px solid ${todo.done ? "#4CAF50" : "#1A73E8"}`,
